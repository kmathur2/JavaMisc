...
public class Merge {
    ...
    private static final String REDUCER_OUTPUT="part";
    ...
    public static void main(String[] args) throws Exception {
        ...
        try{
            ...
            //Initialize paths
            Path currentHour=createPath(args[0]);
            Path mrInput=createPath(args[1]);
            Path mrOutput=createPath(args[2]);
            Path hourlyFile=createPath(args[3]);


            //Step 1 , recover Last Failed Move
            recoverLastFailedMove(mrOutput,hourlyFile);

            //Step 2 , move source files to be aggregated to mr input path
            moveSource(currentHour,mrInput);

            //step 3 , start MR
            startAggregationMR(mrInput,mrOutput);

            //step 4 , move output of MR to short history
            moveMROutput(mrOutput,hourlyFile);

            //step 5 ,  delete output of MR 
            deleteMROutput(mrOutput);

        }catch(IOException ioe){
            logException("IOException "+ioe.getMessage());
        }
